...
import { verifyQrToken } from '../../shared/utils/qr-token.util.js';
import { toAttendanceSessionDTO } from '../../shared/dto/attendance.dto.js';
...
export class AttendanceService {
    ...
    async qrCheckIn(body, libraryId, ipAddress) {
        ...
        if (existingSession) {
            throw new AppError(ERROR_CODES.STUDENT_ALREADY_CHECKED_IN, 'Student is already checked in', 409, { checkInAt: existingSession.checkInAt.toISOString() });
        }
    ...
    async qrCheckOut(body, libraryId, ipAddress) {
        const qrPayload = verifyQrToken(body.qrToken);
        if (!qrPayload) {
            throw new AppError(ERROR_CODES.INVALID_QR_TOKEN, 'Invalid QR token', 400);
        }
        ...
        await this.db.transaction(async (tx) => {
            const updated = await tx
                ...
                .where(and(eq(attendanceSessions.id, openSession.id), eq(attendanceSessions.libraryId, libraryId), isNull(attendanceSessions.checkOutAt)))
                .returning();
            if (updated.length === 0) {
    ...
    async manualCheckIn(body, ctx, ipAddress) {
    ...
    async manualCheckOut(body, ctx, ipAddress) {
    ...
    async correctSession(body, ctx, ipAddress) {
    ...
    async listSessions(query, libraryId) {
        ...
        if (query.date) {
            ...
            conditions.push(and(sql `${attendanceSessions.checkInAt} >= ${startOfDay}`, sql `${attendanceSessions.checkInAt} <= ${endOfDay}`));
        }
        if (query.studentId) {
