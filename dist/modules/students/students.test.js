...
import { cleanupAuthTestData, closeTestResources, createTestApp, seedAuthTestData } from '../../test/helpers.js';
describe('Student module', () => {
    ...
    beforeAll(async () => {
        ...
        tokenA = loginARes.json().data.accessToken;
        ...
        tokenB = loginBRes.json().data.accessToken;
    });
    afterAll(async () => {
        // Cleanup students
    ...
    describe('CRUD operations & Validation', () => {
        ...
        it('regenerates QR token successfully', async () => {
            ...
            const originalQr = originalRes.json().data.qrToken;
            const response = await app.inject({
                method: 'POST',
            ...
            const newQr = newRes.json().data.qrToken;
        ...
        it('soft deletes a student successfully', async () => {
            ...
            const found = listRes.json().data.some((s) => s.id === studentId);
    ...
    describe('Multi-tenant isolation', () => {
        ...
        beforeAll(async () => {
            ...
            studentBId = response.json().data.id;
        });
        ...
        it('does not return Library B student in Library A list/search queries', async () => {
            ...
            const found = response.json().data.some((s) => s.id === studentBId);
            expect(found).toBe(false);
        });
    });
    describe('Search Priority Logic', () => {
        ...
        it('searches by name case-insensitively', async () => {
            ...
            expect(list.map((s) => s.name)).toContain('Alice Kumar');
            expect(list.map((s) => s.name)).toContain('Alice Singh');
        });
    });
});
